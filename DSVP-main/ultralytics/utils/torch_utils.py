...
def model_info(model, detailed=False, verbose=True, imgsz=640):
    ...
    flops = get_flops(model, imgsz=[1280, 736])  # imgsz may be int or list, i.e. imgsz=640 or imgsz=[640, 320]
    fused = " (fused)" if getattr(model, "is_fused", lambda: False)() else ""
    fs = f", {flops:.1f} GFLOPs" if flops else ""
    yaml_file = getattr(model, "yaml_file", "") or getattr(model, "yaml", {}).get("yaml_file", "")
    model_name = Path(yaml_file).stem.replace("dsvp", "DSVP").replace("yolo", "YOLO") or "Model"
...
def get_flops(model, imgsz=640):
    ...
    try:
        ...
        try:
            ...
            # Fall back to the requested image size.
